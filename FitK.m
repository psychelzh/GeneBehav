function [k, m, LL] = FitK(data) %,subject_code)
...
% function plotFit(k,m,subject_code)
% 	global d;
% 
% 	maxd1 = max(d(:,2));
% 	maxd2 = max(d(:,4));
...
% 	t = [0:.1:maxd];
% 	plot(t, 1./(1+k*t), '-k');
% title(['Sub' num2str(subject_code)]);
% 	subplot(1,2,2);
% 	hold off;
% 	for i = 1:size(d,1)
% 		if (i == 2)
...
% 		plot(x,y,style);
% 	end;
%     print(fhandle,'-dpsc','-append','DDfitK'); %% cch added
% close(fhandle)
